...
function handleLevelChange(id, targetLevel, sliderElement) {
  ...
  if (targetLevel > currentLevel && !requirementsMet) {
    state.validation = 'Kies een actie, vink hem af en upload bewijs voordat je naar het volgende niveau gaat.';
...
function init() {
  try {
    ...
    console.error('Kon de Groeituin niet opstarten', error);
    showFatalError('De pagina kon niet laden. Vernieuw of open opnieuw vanaf index.html.');
  }
}
